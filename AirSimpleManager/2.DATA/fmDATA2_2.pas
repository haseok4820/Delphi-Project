...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls, Vcl.ExtCtrls, Vcl.ComCtrls, Vcl.Grids, Vcl.WinXCtrls;
...
type
  TForm_DATA2_2 = class(TForm)
    ...
    procedure Button_TableControlClick(Sender: TObject);
    ...
    procedure StringGrid_TableSetEditText(Sender: TObject; ACol, ARow: Integer; const Value: string);
...
procedure TForm_DATA2_2.Button_TableControlClick(Sender: TObject);
begin
  if (Length(arrFloor) = 0) OR (Length(arrTable) = 0) then
  begin
    DM.GetPopup('먼저 좌석 정보를 입력해주세요.');
    Exit;
  end;
  ...
  try
    with Form_Table do
    begin
      Form_Table.SetBounds(0, Application.MainForm.Top, Width, Height);
      ShowModal;
      Close;
    end;
  finally
      ...
      begin
        ...
        begin
          ...
          begin
            with TableInfo do
            begin
              if Length(arrFloor) = 1 then
                sTableName := IntTOStr(i + 1)
              else
                sTableName := IntTOStr(Length(arrFloor)) + '-' + IntTOStr(i + 1);
              sTableTag := '';
              iTable_X := 0;
              iTable_Y := 0;
              iTable_W := 0;
            ...
            System.Insert(TableInfo, arrTable[High((arrFloor))], Length(arrTable[High((arrFloor))]));
          end;
        end;
      end;
    iFLOOR_FIX: // 수정
      begin
        ...
        begin
          ...
          begin
            ...
            if InputQuery('정보수정[' + IntTOStr(i + 1) + ']', ['구분명(최대 5글자) : ', '좌석수(1 ~ 100) : '], Values) then
            begin
              sFloorName := Values[0];
              iTableCnt := StrToIntDef(Values[1], 1);
              ...
              begin
                k := 0;
                for j := Length(arrTable[i]) to iTableCnt - 1 do
                begin
                  with TableInfo do
                  begin
                    if Length(arrFloor) = 1 then
                      ...
                      sTableName := IntTOStr(i + 1) + '-' + IntTOStr(j);
                    sTableTag := '';
                    iTable_X := 0;
                    iTable_Y := 0;
                    iTable_W := 0;
                  ...
                  System.Insert(TableInfo, arrTable[i], Length(arrTable[i]));
                end;
              end
              else if iTableCnt < Length(arrTable[i]) then
                System.Delete(arrTable[i], iTableCnt, Length(arrTable[i]));
            end;
          end;
        end;
      end;
...
procedure TForm_DATA2_2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  SaveTable;
end;

procedure TForm_DATA2_2.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
const
  c_F2 = 113;
  c_F3 = 114;
  c_F4 = 115;
...
begin
  case Key of
    c_F2:
      FloorEvent(Button_CategoryAdd);
    c_F3:
      FloorEvent(Button_CategoryFix);
    c_F4:
      FloorEvent(Button_CategoryDel);
    c_F5:
      Button_TableControlClick(Button_TableControl);
  end;
end;
      ...
      begin
        ...
        begin
          ...
          begin
            with arrFloor[i] do
            begin
              ParamByName('POSNO').AsIntegers[i] := i + 1;
        ...
        begin
          ...
          DM.DBSQLClear;
          SQL.Text := 'INSERT INTO HTABLE_NM (POSNO, ISEQ, TNAME, POSX, POSY, WIDTH, HEIGHT, COCOLOR) ' +
            ' VALUES(:POSNO, :ISEQ, :TNAME, :POSX, :POSY, :WIDTH, :HEIGHT, :COCOLOR)';
          ...
          begin
            ...
            begin
              if (j >= arrFloor[i].iTableCnt) or (j >= Length(arrTable[i])) then
                Break;

              with arrTable[i, j] do
              begin
                ParamByName('POSNO').AsIntegers[iSeq] := i + 1;
                ParamByName('ISEQ').AsIntegers[iSeq] := j + 1;
                ParamByName('TNAME').AsStrings[iSeq] := DM.AnsiFormat(sTableName, 20);
                ParamByName('COCOLOR').AsStrings[iSeq] := DM.AnsiFormat(sTableTag, 20);
...
begin
  //
  StringGrid_Table.OnSetEditText := nil;
  ...
  if arrFloor[iFloor].iTableCnt <= 0 then
    Exit;
  ...
  StringGrid_Table.OnSetEditText := StringGrid_TableSetEditText;
end;
...
procedure TForm_DATA2_2.StringGrid_TableSetEditText(Sender: TObject; ACol, ARow: Integer; const Value: string);
begin
  //
  with arrTable[iSelFloor, ARow - 1] do
  begin
    case ACol of
      1:
        sTableName := Value;
