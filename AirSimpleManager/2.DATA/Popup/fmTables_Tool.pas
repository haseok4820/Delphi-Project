...
begin
  ...
  if iTag >= 0 then
  begin
    Frame_Edit_X.Edit_Value.Enabled := True;
    Frame_Edit_Y.Edit_Value.Enabled := True;
    Frame_Edit_W.Edit_Value.Enabled := True;
    Frame_Edit_H.Edit_Value.Enabled := True;
    Frame_Edit_Name.Edit_Value.Enabled := True;

    iSelTable := iTag;
    Form_Table.TableClick(Form_Table.FindComponent('Table_' + IntToStr(iTag)) as TPanel);
  end
  else
  begin
    Frame_Edit_X.Edit_Value.Enabled := False;
    Frame_Edit_Y.Edit_Value.Enabled := False;
    Frame_Edit_W.Edit_Value.Enabled := False;
    Frame_Edit_H.Edit_Value.Enabled := False;
    Frame_Edit_Name.Edit_Value.Enabled := False;
  end;
end;
...
begin
  ...
  for i := Low(arrTable[iSelFloor]) to High(arrTable[iSelFloor]) do
    ComboBox_Table.Items.Add(IntToStr(i + 1) + ' : ' + arrTable[iSelFloor, i].sTableName);
...
begin
  ...
  begin
    ...
    if iTable_X + iTable_W > Form_Table.Panel_Table.Width then
      iTable_X := Form_Table.Panel_Table.Width - iTable_W;

    if iTable_Y + iTable_H > Form_Table.Panel_Table.Height then
      iTable_Y := Form_Table.Panel_Table.Height - iTable_H;

    ControlPanel := (Form_Table.FindComponent('Table_' + IntToStr(iSelTable)) as TPanel);
    if ControlPanel <> nil then
    begin
      with ControlPanel do
      begin
        Caption := '[' + Format('%.3d', [iSelTable + 1]) + ']' + sTableName;
        Left := iTable_X;
        Top := iTable_Y;
        Width := iTable_W;
        Height := iTable_H;
    ...
    Frame_Edit_X.Edit_Value.OnChange := nil;
    Frame_Edit_Y.Edit_Value.OnChange := nil;
    Frame_Edit_W.Edit_Value.OnChange := nil;
    Frame_Edit_H.Edit_Value.OnChange := nil;
    ...
    Frame_Edit_X.Edit_Value.OnChange := TableChange;
    Frame_Edit_Y.Edit_Value.OnChange := TableChange;
    Frame_Edit_W.Edit_Value.OnChange := TableChange;
    Frame_Edit_H.Edit_Value.OnChange := TableChange;
  end;
end;
