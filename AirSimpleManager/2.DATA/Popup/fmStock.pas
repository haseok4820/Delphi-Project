...
begin
  ...
  if bTimeStock then
  begin
    sSet := arrStockSet[Low(arrStockSet)].sCODE;
    sUnit := (FindComponent('FrameENV_TIME_0') as TFrame_ENV_Edit).Edit_Value.Text;
    iMaeipAmt := StrToIntDEf((FindComponent('FrameENV_TIME_1') as TFrame_ENV_Edit).Edit_Value.Text, 0);
    iNowMaeip := StrToIntDEf((FindComponent('FrameENV_TIME_2') as TFrame_ENV_Edit).Edit_Value.Text, 0);
  end
  else
  begin
    ...
    try
      ...
      begin
        try
          ...
          ParamByName('STOCKCODE').AsString := DM.AnsiFormat(sCODE, 30);
          ParamByName('STOCKNAME').AsString := DM.AnsiFormat(sName, 200);
          ParamByName('SALEAMT').AsInteger := StrToIntDEf(sAMT, 0);
          ParamByName('GUBUNCODE').AsString := DM.AnsiFormat(sCategory, 2);
          ParamByName('KITPRN').AsString := DM.AnsiFormat(sKit, 4);
          ParamByName('VAT').AsString := DM.AnsiFormat(sType, 4);
          ParamByName('SETCODE').AsString := DM.AnsiFormat(sSet, 2);
          ParamByName('UNIT').AsString := DM.AnsiFormat(sUnit, 6);
          ParamByName('MAEIPAMT').AsInteger := iMaeipAmt;
          ParamByName('NOWMAEIP').AsInteger := iNowMaeip;
          ExecSQL;
        finally
          DM.FDQ.CLose;
          DM.DBDisConnect;
