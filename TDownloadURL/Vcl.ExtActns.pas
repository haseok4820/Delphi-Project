...
type
  ...
  TShowCmd = (scHide, scMaximize, scMinimize, scRestore, scShow, scShowDefault, scShowMaximized, scShowMinimized, scShowMinNoActive,
    scShowNA, scShowNoActivate, scShowNormal);
  ...
  TTabAction = class;
  TValidateTabEvent = procedure(Sender: TTabAction; TabControl: TCustomTabControl; var Enabled: Boolean) of object;
...
{$IF NOT DEFINED(LINUX)}
  ...
  TURLDownloadStatus = (dsFindingResource, dsConnecting, dsRedirecting, dsBeginDownloadData, dsDownloadingData, dsEndDownloadData,
    dsBeginDownloadComponents, dsInstallingComponents, dsEndDownloadComponents, dsUsingCachedCopy, dsSendingRequest, dsClassIDAvailable,
    dsMIMETypeAvailable, dsCacheFileNameAvailable, dsBeginSyncOperation, dsEndSyncOperation, dsBeginUploadData, dsUploadingData,
    dsEndUploadData, dsProtocolClassID, dsEncoding, dsVerifiedMIMETypeAvailable, dsClassInstallLocation, dsDecoding, dsLoadingMIMEHandler,
    dsContentDispositionAttach, dsFilterReportMIMEType, dsCLSIDCanInstantiate, dsIUnKnownAvailable, dsDirectBind, dsRawMIMEType,
    dsProxyDetecting, dsAcceptRanges, dsCookieSent, dsCompactPolicyReceived, dsCookieSuppressed, dsCookieStateUnknown, dsCookieStateAccept,
    dsCookeStateReject, dsCookieStatePrompt, dsCookieStateLeash, dsCookieStateDowngrade, dsPolicyHREF, dsP3PHeader, dsSessionCookieReceived,
    dsPersistentCookieReceived, dsSessionCookiesAllowed);

  TDownLoadURL = class;

  TDownloadProgressEvent = procedure(Sender: TDownLoadURL; Progress, ProgressMax: Cardinal; StatusCode: TURLDownloadStatus;
    StatusText: String; var Cancel: Boolean) of object;
...
{$IF DEFINED(CLR)}
    ...
    function OnDataAvailable(grfBSCF: DWORD; dwSize: DWORD; var formatetc: TFormatEtc; var stgmed: TStgMedium): HResult;
    ...
    function OnProgress(ulProgress: ULONG; ulProgressMax: ULONG; ulStatusCode: ULONG; szStatusText: string): HResult;
...
{$ELSE}
    ...
    function OnDataAvailable(grfBSCF: Cardinal; dwSize: Cardinal; formatetc: PFormatEtc; stgmed: PStgMedium): HResult; stdcall;
...
{$ENDIF}
  public
    procedure ExecuteTarget(Target: TObject); override;
    function ExecuteTargetResult(Target: TObject): HResult;
  published
    property Caption;
    property Enabled;
    property FileName: TFileName read GetFileName write SetFileName;
...
{$IFNDEF CLR}
  ...
  TRecipientType = (rtPrimary, // primary message recipient
  ...
  TMailSendStatus = (mssSuccess, // the message was sent
...
const
  ShowCmds: array [TShowCmd] of Integer = (SW_HIDE, SW_MAXIMIZE, SW_MINIMIZE, SW_RESTORE, SW_SHOW, SW_SHOWDEFAULT, SW_SHOWMAXIMIZED,
...
begin
  ...
  else if IsWindow(FParentHWnd) then
...
{$IF DEFINED(CLR)}
      FHInst := ShellExecute(ParentHWnd, FOperation, FBrowseDlg.FileName, FParameters, FDirectory, ShowCmds[FShowCmd])
{$ELSE}
      FHInst := ShellExecute(ParentHWnd, PChar(FOperation), PChar(FBrowseDlg.FileName), PChar(FParameters), PChar(FDirectory),
        ShowCmds[FShowCmd])
{$ENDIF}
    else
      exit;
...
{$IF DEFINED(CLR)}
    FHInst := ShellExecute(ParentHWnd, FOperation, FFile, FParameters, FDirectory, ShowCmds[FShowCmd]);
{$ELSE}
    FHInst := ShellExecute(ParentHWnd, PChar(FOperation), PChar(FFile), PChar(FParameters), PChar(FDirectory), ShowCmds[FShowCmd]);
{$ENDIF}
  if FHInst <= 32 then
...
procedure TCustomFileRun.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
begin
  Result := ((Control <> nil) and (Target = Control) or (Control = nil) and (Target is TCustomRichEdit)) and TCustomEdit(Target).Focused;
end;
...
procedure TTabAction.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
...
begin
  if FTabControl is TPageControl then
    FEnabled := Assigned(FTabControl) and FTabControl.Enabled and (TPageControl(FTabControl).PageCount > 0)
  else
    FEnabled := Assigned(FTabControl) and FTabControl.Enabled and (TTabControlClass(FTabControl).Tabs.Count > 0);
...
begin
  ...
  begin
    if FWrap then
      Enabled := True
    else if FTabControl is TPageControl then
...
begin
  if NextPage or FWrap then
    SelectNextTab
  else if (Length(FLastTabCaption) > 0) and Assigned(FOnFinish) then
    FOnFinish(Self);
end;
...
begin
  ...
  else if (Length(FOldCaption) > 0) and (AnsiCompareText(Caption, FOldCaption) <> 0) then
  begin
    Caption := FOldCaption;
    FOldCaption := ''
  end;
...
begin
  ...
  Status := MapiSendMail(0, Application.Handle, MapiMessage, MAPI_DIALOG or MAPI_LOGON_UI or MAPI_NEW_SESSION, 0);
...
function TSendMail.MAPISendStatusToMailSendStatus(const Value: ULONG): TMailSendStatus;
begin
  case Value of
    SUCCESS_SUCCESS:
      Result := mssSuccess;
    MAPI_E_AMBIGUOUS_RECIPIENT:
      Result := mssAmbiguousRecipient;
    MAPI_E_ATTACHMENT_NOT_FOUND:
      Result := mssAttachmentNotFound;
    MAPI_E_ATTACHMENT_OPEN_FAILURE:
      Result := mssAttachmentOpenFailure;
    MAPI_E_BAD_RECIPTYPE:
      Result := mssBadRecipType;
    MAPI_E_FAILURE:
      Result := mssFailure;
    MAPI_E_INSUFFICIENT_MEMORY:
      Result := mssInsufficientMemory;
    MAPI_E_INVALID_RECIPS:
      Result := mssInvalidRecips;
    MAPI_E_LOGIN_FAILURE:
      Result := mssLoginFailure;
    MAPI_E_TEXT_TOO_LARGE:
      Result := mssTextTooLarge;
    MAPI_E_TOO_MANY_FILES:
      Result := mssTooManyFiles;
    MAPI_E_TOO_MANY_RECIPIENTS:
      Result := mssTooManyRecipients;
    MAPI_E_UNKNOWN_RECIPIENT:
      Result := mssUnknownRecipient;
    MAPI_E_USER_ABORT:
      Result := mssUserAbort;
  else
...
begin
  Result := (ListControl <> nil) or (ListControl = nil) and (Target is TCustomListControl) and TCustomListControl(Target).Focused;
end;
...
procedure TListControlAction.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
...
begin
  ...
  begin
    ...
    Enabled := inherited IsEnabled(Target) and LC.MultiSelect and (LC.GetCount > 0) and (LC.SelCount < LC.GetCount);
  end
  else
    Enabled := False;
end;
...
begin
  LC := TCustomListControl(Target);
  if Target is TCustomMultiSelectListControl then
    Enabled := inherited IsEnabled(Target) and (LC.GetCount > 0) and (LC.ItemIndex <> -1) and
      (TCustomMultiSelectListControl(LC).SelCount > 0)
  else
    Enabled := inherited IsEnabled(Target) and (LC.GetCount > 0) and (LC.ItemIndex <> -1);
end;
...
procedure TListControlCopySelection.SetDestination(const Value: TCustomListControl);
begin
...
begin
  LC := TCustomListControl(Target);
  if Target is TCustomMultiSelectListControl then
    Enabled := inherited IsEnabled(Target) and (LC.GetCount > 0) and (LC.ItemIndex >= 0) and
      (TCustomMultiSelectListControl(LC).SelCount > 0)
  else
    Enabled := inherited IsEnabled(Target) and (LC.GetCount > 0) and (LC.ItemIndex >= 0) and (LC.ItemIndex >= 0);
end;
...
begin
  Enabled := inherited IsEnabled(Target) and (TCustomListControl(Target).ItemIndex <> -1);
end;
...
const
  ...
  BindStatusCode: array [0 .. 54] of DWORD = (BINDSTATUS_FINDINGRESOURCE, BINDSTATUS_CONNECTING, BINDSTATUS_REDIRECTING,
    BINDSTATUS_BEGINDOWNLOADDATA, BINDSTATUS_DOWNLOADINGDATA, BINDSTATUS_ENDDOWNLOADDATA, BINDSTATUS_BEGINDOWNLOADCOMPONENTS,
    BINDSTATUS_INSTALLINGCOMPONENTS, BINDSTATUS_ENDDOWNLOADCOMPONENTS, BINDSTATUS_USINGCACHEDCOPY, BINDSTATUS_SENDINGREQUEST,
    BINDSTATUS_CLASSIDAVAILABLE, BINDSTATUS_MIMETYPEAVAILABLE, BINDSTATUS_CACHEFILENAMEAVAILABLE, BINDSTATUS_BEGINSYNCOPERATION,
    BINDSTATUS_ENDSYNCOPERATION, BINDSTATUS_BEGINUPLOADDATA, BINDSTATUS_UPLOADINGDATA, BINDSTATUS_ENDUPLOADDATA, BINDSTATUS_PROTOCOLCLASSID,
    ...
    BINDSTATUS_IUNKNOWNAVAILABLE, BINDSTATUS_DIRECTBIND, BINDSTATUS_RAWMIMETYPE, BINDSTATUS_PROXYDETECTING, BINDSTATUS_ACCEPTRANGES,
    ...
    BINDSTATUS_DIRECTBIND, BINDSTATUS_RAWMIMETYPE, BINDSTATUS_PROXYDETECTING, BINDSTATUS_ACCEPTRANGES, BINDSTATUS_COOKIE_SENT,
    BINDSTATUS_COMPACT_POLICY_RECEIVED, BINDSTATUS_COOKIE_SUPPRESSED, BINDSTATUS_COOKIE_STATE_UNKNOWN, BINDSTATUS_COOKIE_STATE_ACCEPT,
    BINDSTATUS_COOKIE_STATE_REJECT, BINDSTATUS_COOKIE_STATE_PROMPT, BINDSTATUS_COOKIE_STATE_LEASH, BINDSTATUS_COOKIE_STATE_DOWNGRADE,
    BINDSTATUS_POLICY_HREF, BINDSTATUS_P3P_HEADER, BINDSTATUS_SESSION_COOKIE_RECEIVED, BINDSTATUS_PERSISTENT_COOKIE_RECEIVED,
...
begin
  for Result := Low(EnumArray) to High(EnumArray) do
    if Value = EnumArray[Result] then
      exit;
...
type
  TUrlMonDownloadToFile = function(Caller: IUnknown; URL: PChar; FileName: PChar; reserved: DWORD; StatusCB: IBindStatusCallback)
    : HResult; stdcall;
{$ENDIF}
...
function TDownLoadURL.ExecuteTargetResult(Target: TObject): HResult;
{$IF DEFINED(CLR)}
var
  UrlMonHandle: THandle;
begin
...
begin
  ...
  if Assigned(UrlMonDownloadToFile) then
  begin
    Result := UrlMonDownloadToFile(nil, PChar(URL), PChar(FileName), 0, Self as IBindStatusCallback);
  end
  else
    raise Exception.CreateResFmt(@SUrlMonDllMissing, [UrlMonLib]);
{$ENDIF}
...
function TDownLoadURL.GetBindInfo(out grfBINDF: DWORD; var bindinfo: TBindInfo): HResult;
{$ELSE}

function TDownLoadURL.GetBindInfo(out grfBINDF: Cardinal; var bindinfo: _tagBINDINFO): HResult;
{$ENDIF}
begin
  Result := E_NOTIMPL;
end;
...
function TDownLoadURL.OnDataAvailable(grfBSCF: DWORD; dwSize: DWORD; var formatetc: TFormatEtc; var stgmed: TStgMedium): HResult;
{$ELSE}

function TDownLoadURL.OnDataAvailable(grfBSCF, dwSize: Cardinal; formatetc: PFormatEtc; stgmed: PStgMedium): HResult;
{$ENDIF}
begin
  Result := E_NOTIMPL;
end;
...
function TDownLoadURL.OnObjectAvailable(const iid: TGUID; punk: IUnknown): HResult;
{$ENDIF}
begin
  Result := E_NOTIMPL;
end;
...
function TDownLoadURL.OnProgress(ulProgress: ULONG; ulProgressMax: ULONG; ulStatusCode: ULONG; szStatusText: string): HResult;
{$ELSE}

function TDownLoadURL.OnProgress(ulProgress, ulProgressMax, ulStatusCode: Cardinal; szStatusText: PWideChar): HResult;
{$ENDIF}
const
...
begin
  ...
  if Assigned(FOnDownloadProgress) then
    FOnDownloadProgress(Self, ulProgress, ulProgressMax, TURLDownloadStatus(DWordEnumToPascalEnumOrd(BindStatusCode, ulStatusCode)),
{$IF DEFINED(CLR)}
...
function TDownLoadURL.OnStartBinding(dwReserved: Cardinal; pib: IBinding): HResult;
begin
...
function TDownLoadURL.OnStopBinding(HResult: HResult; szError: string): HResult;
{$ELSE}

function TDownLoadURL.OnStopBinding(HResult: HResult; szError: PWideChar): HResult;
{$ENDIF}
begin
...
function TSendMail.RecipientTypeToRecipClass(const Value: TRecipientType): ULONG;
begin
  Result := Ord(Value) + 1;
end;
{$ENDIF}
...
procedure TRecipientCollection.Notify(Item: TCollectionItem; Action: TCollectionNotification);
begin
  inherited;
...
procedure TRecipientCollection.SetRecipients(const Index: Integer; const Value: TRecipientItem);
begin
  SetItem(Index, Value);
end;
{$ENDIF}
